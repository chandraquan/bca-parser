...
def parse(file):
    ...
    while i < totalPages - 1:
        if i == 0:
            data_temp = tb.read_pdf_table(file, area=(320, 0, 660, 800), pages=i + 1, pandas_options={'header': None, 'dtype': str}, stream=True)[0]
        else:
            data_temp = tb.read_pdf_table(file, area=(160, 0, 800, 800), pages=i + 1, pandas_options={'header': None, 'dtype': str}, stream=True)[0]
